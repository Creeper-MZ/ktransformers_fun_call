...
class BalanceServeInterface(BackendInterfaceBase):
    ...
    def prefill(self, input_ids: torch.Tensor, is_new: bool, temperature: Optional[float] = None, top_p: Optional[float] = None):
        ...
        # Initialize or update cached state
        if is_new:
            # New conversation - initialize cache
            ...
            new_tokens = input_ids[:, prev_length:]
            new_length = new_tokens.shape[-1]
        ...
        if temperature is not None and temperature != 0:
            ...
            query_add.sample_options.temperature = 0.0001

        if top_p is not None and top_p != 0:
            query_add.sample_options.top_p = top_p
        else:
            query_add.sample_options.top_p = 0.0001

        # Submit to scheduler and prepare queue
        query_id = self.sched_client.add_query(query_add)
        if query_id not in self.queue_map:
            self.queue_map[query_id] = asyncio.Queue(maxsize=self.args.max_new_tokens)
        self.thread_map[thread_id] = query_id

        # Signal completion of prefill phase
        yield None
